...
const RELEASE_REPRESENTATION_DESCRIPTION = '選填：Final 無法表示的 release（例如 MIDI 比 1/64 格點早一個 tick 的 note-off）的表示決定。{decisions:[{id, eventIds, representation: EXTEND_TO_NEXT_GRID|TRUNCATE_TO_PREVIOUS_GRID, reason, attestation:{reviewer, reviewer_kind: human|agent|tool, audio_basis: listening|machine-metric|not-used}, evidence:[{class: primary-symbolic|primary-audio, ref: asset_id 或 source id, locator, finding}]}]}。'
  + '只有 human reviewer 對獨立的 primary 來源（官方譜，或以 listening 聽原曲音訊）所作的證據才會生效；agent／tool、第三方、編碼樣式、音訊指標證據只會記錄、不計入，並保持 PENDING。來源 release 永遠保留在 baseline 與事件紀錄，onset／音高／角色不動，不加 tie、不合併重複音。傳 {decisions:[]} 可只取得逐 release 的分析與證據需求。';
