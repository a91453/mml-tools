...
- `MML_OAUTH_REDIRECT_HOSTS` (optional): comma-separated bare hostnames whose exact HTTPS callbacks may register. Unset, the default is `chatgpt.com,chat.openai.com,claude.ai,claude.com`. A value that is not a bare hostname list refuses to start. The service workspace additionally admits only the exact callback `${MML_PUBLIC_ORIGIN}/studio/`; this does not admit arbitrary callbacks on the service host. To use Google Gemini custom MCP connectors, list Google's three OAuth relay hosts with the defaults: `chatgpt.com,chat.openai.com,claude.ai,claude.com,oauth-redirect.googleusercontent.com,oauth-redirect-sandbox.googleusercontent.com,oauth-redirect-test.googleusercontent.com` (the value replaces the default list, so keep the other four). Gemini registers six callbacks spread over the production, sandbox and test relays in one request, and one unlisted callback refuses the whole registration. They are not defaults because each relay forwards to whichever Google project or connector the callback path names; each is admitted only as that exact host, never as `*.googleusercontent.com`.
