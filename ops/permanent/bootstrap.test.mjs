...
test('empty volume, verified cached start, fail-closed corruption, durable recovery and legacy preservation', async () => {
  ...
  try {
    ...
    const first = await bootstrapStudio(config);
    ...
    const second = await bootstrapStudio({ ...config, fetchArchive: async () => { throw Error('Network must not be used'); } });
    ...
    const recovered = await bootstrapStudio({ ...config, recoverCorrupt: true });
