...
self.onmessage = async ({ data }) => {
  try {
    ...
    else if (['newWorkspace', 'intake', 'intakeMidi', 'analyzeWorkspace', 'invalidate', 'importWorkspace', 'recordReview', 'recordLeadEvidence', 'recordLeadPromotionEvidence', 'recordAcceptedDecision', 'clearAcceptedDecisions', 'recordAcceptance', 'generateFinalDelivery', 'applyFinalDelivery', 'previewMobileAdaptation', 'applyWorkspaceMobileAdaptation', 'clearMobileAdaptation'].includes(data.action)) result = model[data.action](...data.args);
