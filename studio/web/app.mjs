...
function timbrePreviewCard() {
  ...
  return `<div class="card preview-card" id="timbre-preview"><div class="attempt-head"><h3>遊戲音色試聽</h3><span class="badge na">模擬試聽</span></div>
    ...
    <div class="preview-controls"><label>全部角色音色${select('id="preview-program"', all, '按播放後載入音色清單')}</label>${Object.keys(ROLE_GROUPS).map(group => `<label>${ROLE_GROUP_LABELS[group]}${select(`data-preview-group="${group}"`, groupChoice(preview.choices, group), '按播放後載入音色清單')}</label>`).join('')}
