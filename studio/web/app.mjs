...
async function commit(next) {
  ...
  catch (error) { message(`尚未儲存：${error.message}。可先匯出專案備份。`, true); }
  render();
}
...
function bind() {
  ...
  document.querySelectorAll('[data-core3]').forEach(form=>form.onsubmit=event=>{event.preventDefault();const data=Object.fromEntries(new FormData(form));run(async()=>{
    if(!data.reason?.trim()||!data.evidence?.trim())throw Error('需要變动理由與證據');const change=report.core3.unapproved[Number(form.dataset.core3)];const next=structuredClone(workspace);next.core3Approvals.push({eventId:change.eventId,type:change.type,reason:data.reason,evidence:[data.evidence],revision:workspace.revision});await commit(next);
  });});
  ...
  $('#request-audio').onclick=()=>run(async()=>{
    ...
    const {requestAudioAlignment}=await import('./audio-client.mjs');uploadController=new AbortController();$('#cancel-audio').disabled=false;$('#audio-progress').textContent='Audio Alignment 執行中…';
  ...
  $('#audio-report').onchange=()=>{const file=$('#audio-report').files[0];if(file)run(async()=>{if(file.size>4194304)throw Error('Report exceeds 4 MiB');const next=structuredClone(workspace);next.audio={revision:workspace.revision,report:JSON.parse(await file.text())};await commit(next);});};
...
try {
  ...
  if(workspace.canonicalKey&&workspace.canonicalKey!==canonicalKey)workspace=await call('invalidate',workspace);
  workspace.canonicalKey=canonicalKey;
