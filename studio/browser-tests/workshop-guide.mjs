...
// panel, zh-Hant like the Workshop, drawn in the Workshop's theme, and no console
...
export async function runWorkshopGuideChecks({ browser, base, profile }) {
  ...
  try {
    ...
    await ready();
    // The theme is the Workshop's; a language an older Workshop stored changes nothing.
    ...
    assert.equal(await page.evaluate(() => document.documentElement.lang), 'zh-Hant');
    await page.evaluate(() => localStorage.removeItem('studio-workshop/ui'));
