...
// load (the engine is still booting), or the read of the bank kept in the
// store (the engine is ready and the kept bank has been asked for). Each is
// installed with page.addInitScript and armed for one load by a
// sessionStorage flag, holdProcessor or holdKeptBankRead, set before the
// reload; window.processorHold / window.keptBankReadHold then report how
// many requests are held and release them. Playwright does not route an
