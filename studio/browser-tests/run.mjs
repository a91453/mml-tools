...
import { runGameStyleBankChecks, runWorkshopGameStyleChecks } from './game-style-bank.mjs';
...
try {
  for(const profile of [
    ...
    try {
      ...
      // It refuses the Worker's own module request with a route, and routing a
      ...
      await runWorkshopGuideChecks({browser,base,profile});
      await runWorkshopOriginChecks({browser,base,profile});
      ...
      results.push({profile:profile.name,status:'PASS',checks:['Files picker','source pickers carry no accept list','Final generation panel','exact Final copy/download/display identity','role-body copy labelled','clipboard fallback exact string','attempt vs applied delivery separated','P1 character-count disclaimer','superseded Final not copyable','refused attempt beside valid pasted delivery','whole-score exports agree','pasted-whitespace delivery exports agree','Final reduction preview is read-only','reduction accounting buckets visible','pending reduction material never presented as applied','event-level reduction decision accepted','reduction apply certifies no gate','reduction IndexedDB reload','re-preview carries the applied decisions','reduction rollback to parent candidate','local MML/MusicXML','full review workflow','state separation','exact clipboard payload','IndexedDB reload','boot busy signal','busy-window two-waiter FIFO','boot waiter drain','project-scoped queue','stable Core3 evidence IDs','Worker failure and recovery','unsaved failure state','IndexedDB stale-token rejection','Final pitch boundary','settled state never ANALYSIS_RUNNING','source-aware micro-timing gate visible','revision invalidation','unsupported fail closed','no implicit uploads','responsive layout','offline module graph','Raw MIDI file picker','binary intake and source digest','Raw MIDI section structure','Core3 vs Full6 separation','percussion visible and never pitched','byte-exact IndexedDB reload','exact rational timing through the browser','superseded MIDI request discarded','malformed MIDI fails visibly','same-file reselect after failure','no Raw MIDI upload','save-state indicator','whole-library ZIP export and restore','IndexedDB v1 to v2 migration','truncated bank refused before it is kept','timbre card leaves loading after an unparsable bank','timbre card leaves loading when the synth never reports ready','player readback through the real engine','incomplete playback not recordable','free default bank: nothing downloaded on page load','first play shows the download notice and progress','downloaded bank and derived subset verified by SHA-256','default bank plays labelled, with the 11 game instrument names','cached subset replays after reload with no second request','altered or unreachable default bank refused with a visible message','truncated user bank refused before it is kept','kept unparsable bank ends loading with a visible message and play retries','user bank overrides the default bank','a refused bank pick leaves the playing engine and the kept bank','a bank change during a restart leaves no playback on the old engine','an engine built for a bank picked away is not installed','the page names the bank the store keeps and the engine plays','a bank change stops a listening playback and says so','overlapping bank picks: the last pick is kept and shown','listen link import without autoplay','listen link cleared from the address bar','listening session separate from projects','play from bar/time/marker through the real scheduler','stop and replay from the same point','changed bars and A/B ranged playback','listening notes, copy for AI and project mirroring','listening session deletion','decision composer preview writes nothing','previewed decision accepted exactly','accepted decision moves no gate','workshop opens a Studio MML as a copy','workshop language switch keeps the score','workshop import dialog in the page language','workshop dark/light theme on load','workshop unparsable bank ends loading and the next pick loads','workshop error line shows bank bytes as text','workshop pick whose check runs out of time is refused and never loaded','workshop pick whose checker never loads is refused and never loaded','workshop overlapping bank picks: the overtaken pick is neither kept nor loaded','workshop bank picked during engine boot is kept','workshop synth that never reports ready ends each load','workshop roll edit with undo/redo','workshop 3MLE export/import','workshop MusicXML and .mxl import to the same score','workshop import names the song without the file extension','workshop import trims leading bars with the meters and marks','workshop WAV export through the render worker','workshop video preview','workshop edit returns as an unverified derived candidate','workshop library song drops the previous Studio source label and return target',...(profile.engineName==='chromium'?['boot survives a model request the Worker could not fetch','a model that never fetches ends boot with its reason','boot asks again after a Worker script request that failed','what a new Worker cannot fix is not retried','a listen link survives a listen model request the Worker could not fetch','a listen link opened offline opens and analysis goes on','a listen link that could not open never reports that it did','a listen link asks again after a Worker script request that failed','workshop offers a waiting release','workshop left stale by a Studio-tab update refuses exports and Studio reads/writes until reloaded','workshop apply refused while edits exist only in memory','workshop apply reloads only its own tab and leaves Studio stale']:[])]});
    } catch(error) {
