// Lead Demotion Gate (MASTER_RULES.md §4, SOURCE_POLICY.md §4).
//
// This module is the one place a Lead demotion is judged, and -- since the
// G11-D residual hardening -- the one place a Lead evidence citation is bound to
// the event it claims to describe. Every caller (the pre-G11-D Studio Web Lead
// evidence path, the G11-D application and the G11-D downstream review) reaches
// the same `sourceIdentityBlockers()`; none carries a private copy whose
// semantics could drift. A caller may run the binding *before* the gate to
// short-circuit, but the gate runs it again itself, so a caller that forgets
