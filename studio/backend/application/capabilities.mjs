  ...
  return freeze({
    ...
    runs: freeze({
      ...
      refuses: freeze([
        ...
        'naming an operation for a finalize the Final emitter refused unless EVENT_ROLE_UNASSIGNED is the sole blocking diagnostic: a new explicitly reviewed Final reduction can assign the role then; otherwise FINALIZE_BLOCKED and the technical request it leaves NOT_RUN list no operation. FINALIZE_BLOCKED carries the emitter\'s blocking diagnostic codes in detail.emitter_blockers',
        'reporting machine delivery ready, or reading a PENDING as a Final, when the Final emitter run on exactly what would be delivered with the options delivery uses writes no Final: machine delivery records FINAL_EMISSION_REFUSED or FINAL_EMISSION_PENDING as its finalEmission entry with the emitter\'s own status and diagnostics unchanged, and a finalEmission request lists no operation; it still blocks',
