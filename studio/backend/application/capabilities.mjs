  ...
  return freeze({
    ...
    runs: freeze({
      ...
      refuses: freeze([
        ...
        'naming an operation for a readiness blocker none in this build answers: MICRO_TIMING_BOUNDARY_NOT_FINAL_REPRESENTABLE (an onset, a rest boundary, or a release under a keep claim or with no valid representation, that Final cannot reach) and MICRO_TIMING_SOURCE_SUPPORTED_NOT_FINAL_REPRESENTABLE (a source-supported sub-grid interval no Final token can carry) are stated in the review request\'s missing, and a microTiming request that carries only them lists no operation; nor does a microTiming request while no note release awaits a representation decision (readiness.gates.microTiming.releaseTiming.decisionRequiredCount 0), because release representation moves only such a release; it still blocks',
        'naming a gate operation for a finalize the Final emitter refused: the emitter runs only when no gate finalize grades before emission blocks delivery, so FINALIZE_BLOCKED and the technical request it leaves NOT_RUN list only the operations EMITTER_REFUSAL_OPERATIONS names for the emitter\'s codes (EVENT_ROLE_UNASSIGNED: planFinalReduction and applyFinalReduction, a reduction that places or omits material kept outside the six roles) and none for any other code, and FINALIZE_BLOCKED carries the emitter\'s blocking diagnostic codes in detail.emitter_blockers',
        'reporting machine delivery ready, or reading a PENDING as a Final, when the Final emitter run on exactly what would be delivered with the options delivery uses writes no Final: machine delivery records FINAL_EMISSION_REFUSED or FINAL_EMISSION_PENDING as its finalEmission entry with the emitter\'s own status and diagnostics unchanged, and a finalEmission request lists only what EVENT_ROLE_UNASSIGNED names at a finalize the emitter refused, and no operation otherwise; it still blocks',
