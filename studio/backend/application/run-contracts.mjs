...
export const READINESS_GATE_OPERATIONS = freeze({
  ...
  // open one is answered in the Canonical source itself, and while no release
  // awaits a representation decision these operations are not offered at all
  // (READINESS_GATE_OPERATION_REACH below). A preserved one
...
/**
 * Whether a gate's hinted operations can act on this candidate at all, read
 * from the gate's own report.
 *
 * Release representation, the only operation this build has for microTiming,
 * moves a note release that awaits a representation decision and refuses every
 * other event (RELEASE_EVENT_IS_NOT_A_REPRESENTATION_TARGET,
 * RELEASE_EVENT_HAS_A_SOURCE_SUPPORTED_KEEP_CLAIM,
 * RELEASE_REPRESENTATION_NOT_VALID_FOR_EVENT). G10 counts those releases in
 * `releaseTiming.decisionRequiredCount`. While there are none, the gate's
 * operations cannot change any blocker it carries -- an unclassified sub-grid
 * note, a gap after a release Final can express, technical residue, a policy
 * or stream finding -- so a request for it names no operation, and `missing`
 * states every blocker READINESS_BLOCKER_WITHOUT_OPERATION does not already
 * state, in the words below. Like the tables above this decides nothing: the
 * gate blocks exactly as readiness says.
 */
export const READINESS_GATE_OPERATION_REACH = freeze({
  microTiming: freeze({
    reaches: gate => (gate?.releaseTiming?.decisionRequiredCount ?? 0) > 0,
    unreached: codes => [
      `${codes.join(', ')}: readiness.gates.microTiming reports no note release awaiting a representation decision (releaseTiming.decisionRequiredCount is 0, or no release analysis ran), and release representation, the only operation this build has for this gate, moves only such a release and refuses every other event (RELEASE_EVENT_IS_NOT_A_REPRESENTATION_TARGET), so planMobileAdaptation and applyMobileAdaptation.release_representation are not offered: neither can change these blockers.`,
      codes.includes('MICRO_TIMING_CLASSIFICATION_UNKNOWN')
        ? ' An unclassified sub-grid interval with no such release behind it (a sub-grid note, or a gap after a release Final can express; readiness.gates.microTiming.unknownIntervals names each) is classified only by a micro-timing decision the Canonical project carries, on its own evidence (SOURCE_POLICY), and no operation in this build records one: applyDecisions and applyFinalReduction change no event timing, and the finalize-time Technical Timing Repair never runs past a blocked microTiming gate. It is answered in the Canonical source itself.'
        : '',
