...
This repository is published as a whole, Git history included: source, tests, project documentation and operational records (`ops/`, `railway/`). It holds no credentials; deployment secrets live in the deployment's own environment variables and secret stores, and `scripts/scan-secrets.mjs` checks every tracked file on each pull request and push.
...
Components under their own license keep it; the MIT License of this repository does not apply to them.
