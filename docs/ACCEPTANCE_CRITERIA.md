...
- each non-empty role within current official character limit;
- official Tempo/Volume/note-length/pitch limits enforced;
- no zero duration;
- no shell/Markdown/label contamination in paste-ready output;
- fragile Final-forbidden syntax canonicalized;
- exact timing representation preserved.
...
Required:
- Source-Faithful Baseline exists or equivalent traceable source map is available;
- important Lead, harmony, bass, counter/texture and form events are attributable to sources;
- candidate diff against baseline is available;
- removals, additions, role moves and pitch/onset/duration edits are explainable.
...
- Tempo Map agrees with the candidate/source decision;
...
Required:
- compare candidate to accepted previous version when available;
- detect unintended Lead/Core3/source drift;
- permanent historical regressions are checked when applicable;
