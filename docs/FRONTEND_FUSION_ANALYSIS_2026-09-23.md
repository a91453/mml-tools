...
### 14.9 工作坊只保留繁體中文（2026-09-26）

擁有者決定：工作坊只有擁有者自己使用，介面**只保留繁體中文**，取代 §14.6、§14.8 的「工作坊四語系維持不變」。

- **刪除**：`i18n/en.mjs`、`ja.mjs`、`ko.mjs` 三份語言表；`lang.mjs`（語言選單與切換語言時暫存樂譜再重新載入）；設定裡的
  「語言 (Language)」選單與它的兩個 key；`boot.js` 的瀏覽器語言偵測與 `data-i18n-pending`（及 `workshop.css` 對應的隱藏規則）；
  說明頁的「目前只有中文版」提示（`guide.mjs` 不再讀偏好）。
- **保留**：`i18n.t()` 與 `i18n/zh-Hant.mjs` 文字表、`i18n-page.mjs` 的 `data-i18n` 掛鉤。文字仍集中在一份表裡，頁面與模組共用；
  拆掉這層要改幾百處呼叫，風險大於好處。`i18n.mjs` 拿掉動態載入、複數規則與韓文助詞（繁中表都用不到）。
- **連帶**：`getLocale()` 固定回傳 `zh-Hant`（時間、數字格式照舊）；舊版非 UTF-8 檔案一律以 Big5 讀（原本就是繁中的預設）；
  Studio 與工作坊共用的偏好 `studio-workshop/ui` 若還留著舊的 `lang` 欄位，兩邊都忽略、也不刪。
- **驗證**：`studio/tests/workshop-i18n.test.mjs` 改為只有一份表（`i18n/` 只有 `zh-Hant.mjs`、沒有 `lang.mjs`、沒有語言選單、
  `boot.js` 不偵測語言、每一項都是非空字串且只有 `{name}` 佔位符）；瀏覽器 `workshop.mjs` 的語言切換段改為確認沒有語言選單、
  存著 `lang: 'ja'` 重新載入仍是繁中；`web-theme.mjs` 用日文瀏覽器開工作坊仍是繁中；`workshop-guide.mjs` 不再有中文版提示。
