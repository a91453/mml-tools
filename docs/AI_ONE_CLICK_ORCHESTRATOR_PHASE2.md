...
writer on it, and a writer record that names a revision other than the run's
names nobody. Either reads as not this application's, so such a retry is refused
